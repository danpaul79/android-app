...
class MorningActionReceiver : BroadcastReceiver() {
    ...
    companion object {
        ...
        const val ACTION_REVIEW_DONE = "com.example.aicompanion.ACTION_REVIEW_DONE"
        const val ACTION_REVIEW_SECONDARY = "com.example.aicompanion.ACTION_REVIEW_SECONDARY"
        const val ACTION_REVIEW_SKIP = "com.example.aicompanion.ACTION_REVIEW_SKIP"
        const val EXTRA_TASK_ID = "task_id"
        const val EXTRA_NOTIFICATION_ID = "notification_id"
        const val EXTRA_REVIEW_TYPE = "review_type"
    }
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (action in setOf(ACTION_REVIEW_DONE, ACTION_REVIEW_SECONDARY, ACTION_REVIEW_SKIP)) {
            val taskId = intent.getLongExtra(EXTRA_TASK_ID, -1L)
            val notifId = intent.getIntExtra(EXTRA_NOTIFICATION_ID, -1)
            val reviewType = intent.getStringExtra(EXTRA_REVIEW_TYPE)

            val manager = context.getSystemService(NotificationManager::class.java)
            if (notifId >= 0) manager.cancel(notifId)

            if (action == ACTION_REVIEW_SKIP) return
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    when (action) {
                        ACTION_REVIEW_DONE -> repo.toggleCompleted(taskId, true)
                        ACTION_REVIEW_SECONDARY -> {
                            if (reviewType == ReviewType.STALE.name) {
                                repo.trashTask(taskId)
                            } else {
                                repo.removeTagFromNotes(taskId, "waiting-for")
                            }
                        }
                    }
                } finally {
